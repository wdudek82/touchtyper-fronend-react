...
class TypingExercise extends Component {
  ...
  createInitialTokenSpans = (tokensData, charSpans) => {
    ...
    return tokensData.tokens.map((token, index) => {
      ...
      return this.wrapInTokenSpan(start, token.length, index === 0, charSpans, token);
    });
  };
  ...
  render() {
    return (
      <div>
        ...
        <button
          type="button"
          style={{ background: `${this.state.isPrivate ? 'green' : 'red'}` }}
          onClick={() => {
            this.setState((prevState) => ({ isPrivate: !prevState.isPrivate }));
          }}
        >
          {this.state.isPrivate ? 'Public' : 'Private'}
        </button>
        ...
        {/*<Statistics*/}
          {/*typedText={this.state.typedText}*/}
          {/*mistakesIndexes={this.state.mistakesIndexes}*/}
